...
let totalPrice (onSales : OnSale list) (iceSold : IceSold) =
  let factor =
    onSales
    |> List.tryFind (fun onSale -> onSale.Flavour = iceSold.Flavour && onSale.Weekday = iceSold.Weekday)
    |> Option.map (fun onSale -> onSale.Factor)
    |> Option.defaultValue 1.0


  priceFor iceSold.Flavour * priceMultiplier iceSold.Size * factor
